...
function popupHtml(stop) {
  ...
  const cost =
    stop.cost === "free"
      ...
      : stop.cost === "paid"
        ? `<span class="popup__cost popup__cost--paid">Campground (paid)</span>`
        : stop.cost
          ? `<span class="popup__cost">${escapeHtml(stop.cost)}</span>`
          : "";
  const freeAlt = stop.freeAlternative?.url
    ? `<p class="popup__alt">Free backup: <a href="${escapeHtml(stop.freeAlternative.url)}" target="_blank" rel="noopener">${escapeHtml(stop.freeAlternative.name || "iOverlander")}</a></p>`
    : stop.paidBackup?.url
      ? `<p class="popup__alt">Paid backup: <a href="${escapeHtml(stop.paidBackup.url)}" target="_blank" rel="noopener">${escapeHtml(stop.paidBackup.name || "iOverlander")}</a></p>`
      : "";
  return `
    <div class="popup">
...
function resolveVersionId(catalog) {
  ...
  // Only honor ?v= when explicitly pinned by the version picker (?pin=1).
  // Bare ?v= from older auto-writes is ignored so activeId can surface.
  if (
    fromQuery &&
    params.get("pin") === "1" &&
    catalog.versions.some((v) => v.id === fromQuery)
  ) {
    return fromQuery;
  }
...
async function loadVersion(versionId, { pin = false } = {}) {
  ...
  const [route, stopsJson] = await Promise.all([
    fetch(`${base}/route.geojson?t=${Date.now()}`).then((r) => {
      if (!r.ok) throw new Error(`Missing route for ${versionId}`);
      return r.json();
    }),
    fetch(`${base}/stops.json?t=${Date.now()}`).then((r) => {
  ...
  if (pin) {
    ...
    // Drop stale auto-written ?v= so activeId can take effect next refresh.
...
document.getElementById("version-select").addEventListener("change", async (event) => {
  ...
  try {
    await loadVersion(next, { pin: true });
  } catch (err) {
    console.error(err);
...
map.on("load", async () => {
  ...
  versionCatalog = await fetch(`versions/index.json?t=${Date.now()}`).then((r) => r.json());
  ...
  await loadVersion(versionId, { pin: false });
});
